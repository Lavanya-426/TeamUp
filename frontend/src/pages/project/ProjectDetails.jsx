...
function ProjectDetails() {
  ...
  useEffect(() => {
    ...
    // fetch teams
    fetch(`${import.meta.env.VITE_API_URL}/api/teams/${id}`, {
      headers: { Authorization: token },
    })
      .then((res) => res.json())
      .then(setTeams);
  ...
  return (
    <Layout>
      <div className="p-6">
        {/* PROJECT DETAILS */}
        <h2 className="text-2xl font-bold mb-4">{project.courseName}</h2>
        ...
        {/* TEAMS LIST */}
        <h3 className="text-xl font-semibold mt-6 mb-3">Available Teams</h3>

        {teams.length === 0 && <p>No teams yet</p>}

        {teams.map((team) => (
          <div key={team._id} className="bg-white p-4 rounded shadow mb-3">
            <p>
              <b>Leader:</b> {team.leader.username}
            </p>
            <p>
              <b>Members:</b> {team.members.length}
            </p>

            <button className="btn mt-2" onClick={() => sendRequest(team._id)}>
              Send Request
            </button>
          </div>
        ))}
